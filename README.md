...
- 🎨 **变体样式**：支持 `v:primary`、`v:muted`、`v:highlight` 等变体类名，统一控制组件视觉风格。
...
└── tests/
    ├── helpers/
    │   └── dom-mock.js     # 测试用最小化 DOM mock
    ├── parser.test.js      # 解析器测试
    ├── renderer.test.js    # 渲染器测试
    └── useTokUIStream.test.js  # 流式组合式函数测试
```
...
- DSL 解析器：静态解析、容器嵌套、同类型容器嵌套、多属性、同级节点、流式分片与布局
- DOM 渲染器：变体类名生成、嵌套同类型容器关闭
- 流式组合式函数：SSE 连接错误状态上报
